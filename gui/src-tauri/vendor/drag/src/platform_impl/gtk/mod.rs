...
use crate::{CursorPosition, DragItem, DragMode, DragOperation, DragResult, Error, Image, Options};
use gdkx11::{
    gdk,
...
fn on_drop_performed<F: Fn(DragResult, CursorPosition) + Send + 'static>(
    ...
    drag_context.connect_drop_performed(move |context, _| {
        ...
        callback(DragResult::Dropped(DragOperation::Unknown), get_cursor_position(&window).unwrap());
    });
}
